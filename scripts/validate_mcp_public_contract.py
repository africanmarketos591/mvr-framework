...
def validate_local() -> None:
    ...
    governed_revenue_release = load_json("release-evidence/2026-08-20-governed-revenue-closure/RELEASE_EVIDENCE.json")
    ...
    grok_bot = recipe.get("grok_bot_early_beta", {})
    require(grok_bot.get("status") == "activation_playbook_published_host_execution_not_yet_certified", "Grok Bot playbook status")
    require(grok_bot.get("connector", {}).get("verification") == "not_yet_operator_verified_in_grok_bot", "Grok Bot non-certification boundary")
    require(grok_bot.get("connector", {}).get("endpoint") == "https://africanmarketos.com/mcp/preflight" and grok_bot.get("connector", {}).get("expected_tool_count") == 5, "Grok Bot bounded connector profile")
    require(set(grok_bot.get("team_pattern", {})) == {"coordinator_bot", "evidence_bot", "preflight_bot", "synthesis_bot"}, "Grok Bot team pattern")
    require("mvr_first_call" in grok_bot.get("client_instruction", "") and "mcp_next_tool" in grok_bot.get("client_instruction", ""), "Grok Bot handoff instruction")
    ...
    require(grok_release.get("deployment_revision") == "2026-08-20.grok-bot-activation.1", "Grok Bot release revision must remain an immutable historical record")
    ...
    require(governed_revenue_release.get("authority_boundaries", {}).get("provider_success_required_for_paid") is True, "governed revenue provider authority boundary")
    require(governed_revenue_release.get("authority_boundaries", {}).get("new_public_offer_created") is False, "governed revenue public-offer boundary")
...
def validate_live() -> None:
    ...
    require(recipe.get("grok_bot_early_beta", {}).get("status") == "activation_playbook_published_host_execution_not_yet_certified", "live Grok Bot playbook missing")
    require(recipe.get("grok_bot_early_beta", {}).get("connector", {}).get("verification") == "not_yet_operator_verified_in_grok_bot", "live Grok Bot certification overclaim")
