...
Application revision: `2026-09-06.engine-contract.6`. This documents technical contracts, not scientific validation or institutional approval.
...
The broad REST catalogue deliberately includes operations with incomplete request schemas and differing maturity labels. It is not an assertion that every route is stable or a finished SDK workflow. The keyless first-call contract, licensed DecisionCheckRequest/EvidenceItem definitions, evidence-completeness schema, and consent-gated outcome-ledger schema already exist. This release adds explicit inputs and boundaries for calibration-readiness export, snapshot lock, and benchmark-delta. It does not promote their maturity labels.
