...
Every item carrying claim prose or a load-bearing dependency needs a source-to-target link. Omission returns claim_review.status=missing_claim_linkage and pauses the chain, just as explicit counterevidence pauses it for review. Every declared target claim needs a link too. Context-only links are valid background declarations but cannot satisfy a load-bearing dependency. linkage_gaps identifies the submitted items needing mapping; the collection plan includes a review_claim_linkage task. Clearing load_bearing does not exempt source prose. Metadata-only, non-load-bearing inventories may still pass structural coverage with unassessed_content; this is not assessment of a target. A caller can still lie about relationships or omit the evidence itself: this gate enforces declared linkage, not honesty or automatic contradiction discovery.
